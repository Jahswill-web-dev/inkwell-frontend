...
const valid = { notes: " Notes ", working_title: " Title ", target_audience: " Writers ", article_goal: "inform_and_inspire" as const };
...
describe("article schemas", () => {
  ...
  it("trims input and rejects whitespace and limits", () => {
    expect(articleInputSchema.parse(valid)).toMatchObject({ notes: "Notes", working_title: "Title", target_audience: "Writers" });
  ...
  it("requires a non-empty patch and computes changed fields", () => {
    ...
    expect(changedArticleFields(article, { notes: "Notes", workingTitle: "New title", targetAudience: "Writers", articleGoal: "inform_and_inspire" })).toEqual({ working_title: "New title" });
  });
});
